...
1. **`scripts/*.sh`** — 全操作の実体。すべて `_lib.sh` を source する。
   ...
   - write 系: `wiki-init` / `wiki-index-upsert` / `wiki-log` / `wiki-move`
   - 複雑なテキスト操作（index 編集・move・validate）は bash 内のヒートドキュメント Python で実装し、値は環境変数で受け渡す。
...
- **移行スクリプト** — 既存データを一括で新形式へ書き換える `scripts/`（リンク全置換のような機械的変換はこちら。`wiki-move.sh` の link-rewrite が参考実装）。冪等に作り、適用前後で `wiki-validate.sh` が通ること。
...
- 形式やスキーマを変えたら `config.yml` にスキーマ版を持たせる等で「どの版のデータか」を判別可能にすると、移行の検出が確実になる（現状は版管理なし。導入を推奨）。
