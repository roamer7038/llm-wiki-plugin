...
- **リンク** = 絶対形 `[[scope/page_type/slug]]` が正本。短縮形 `[[slug]]` は同一ディレクトリ内のみ許可。
...
| **再編** | move のリンク書換え・index 更新・log | move 起動・移動先判断 |
...
蓄積の主入口はこの ingest 1 本。加えて2つの補助経路がある。
...
| `scripts/wiki-{init,index-upsert,log,move}.sh` | write 系（flock 保護） |
| `scripts/wiki-{path,search,validate,slug}.sh` | read 系 |
