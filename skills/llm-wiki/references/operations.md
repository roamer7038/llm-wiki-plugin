...
**本流（親）はオーケストレーションのみ。** read・判断フェーズは必ず `agents/` のサブエージェントに委譲し、本流はユーザ確認と書き込み（flock 直列化）に専念する。

原則:

- **委譲するのは read・判断のみ。** ページ／index／log／リンクの書き込みは必ず本流で行う（`wiki.lock` の単一グローバル flock で直列化されるため、並列書き込みは競合するだけ）。例外として `wiki-source-analyst` だけは `<scope>/raw/` への保存を内部で行う（raw はリンクグラフの簿記でなく flock 対象外）。
- **ユーザ確認は本流に残す。** エージェントは提案／findings／回答を返すだけ。承認・修正適用・queries 還元の判断は本流。
- エージェントもスクリプトを呼ぶ（決定論操作を再実装しない）。フックからエージェントを起動しない。

| エージェント | 用途 | 委譲基準 | 並列 | 返却 |
|---|---|---|---|---|
| `wiki-source-analyst` | ingest の取得〜ページ案 | **常に委譲**（ソースの大小不問・インライン処理なし） | ソース単位（複数なら並列） | `## 取り込み提案` |
| `wiki-auditor` | lint の意味的監査 | **常に委譲**（単一スコープでも起動） | スコープ単位（全体なら並列） | `## 監査結果` |
| `wiki-researcher` | query の大規模調査 | **常に委譲**（Wiki ページを読む必要があるあらゆる問い） | 通常単一 | `## 回答`＋`## 引用` |

返却ブロックの厳密な形は各エージェント定義（`agents/*.md`）の「Output format」を正本とする。本流はそのブロックを受けて、ingest なら下記 Ingest 手順 4〜7、lint なら Lint 手順 3、query なら Query 手順 4 を実行する。
