...
read が重い／独立並列にできるフェーズは `agents/` のサブエージェントに出し、**本流はユーザ確認と書き込み（flock 直列化）を担う**。原則:
...
|---|---|---|---|---|
| `wiki-source-analyst` | ingest の取得〜ページ案 | 長い記事/PDF・複数ソース。小さい単一ソースは委譲しない | ソース単位 | `## 取り込み提案` |
| `wiki-auditor` | lint の意味的監査 | 複数トピック／全体。単一スコープは本流可 | トピック単位 | `## 監査結果` |
| `wiki-researcher` | query の大規模調査 | 多ホップ・複数トピック横断・グラフ全体合成。単一ページ参照は委譲しない | 通常単一 | `## 回答`＋`## 引用` |
