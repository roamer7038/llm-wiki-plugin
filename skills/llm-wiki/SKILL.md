...
### Ingest（取り込み・確認付き）
1. 原典を `<scope>/raw/` に保存。`source_id`（URL 正規化 or ファイル hash）を控える。
2. `wiki-search.sh` で重複確認（`source_id`・title・aliases）。既存なら更新。
3. 要点・種別・スコープ案をユーザに提示し**確認を取る**。
...
### Move / Rename / 再編
- 必ず `bash ${CLAUDE_PLUGIN_ROOT}/scripts/wiki-move.sh <from-ref> <to-ref>` を使う（inbound リンク書換え・index・log を一括処理）。手で mv しない。
- ref 形式は `scope/page_type/slug`。
